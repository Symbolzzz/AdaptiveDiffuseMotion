...
class JsonWriter():
    def __init__(self):
        # ARKit blendshape names, corresponding to 51 blendshape values
        self.names = [
    ...
    def write(self, X, ofile):
        ...
        # Assume X is of shape (nframes, 51) where nframes is the number of frames
        nframes = X.shape[0]
        offset = 1 / 60
        # Generate frame data
        for i in range(nframes):
            frame_data = {
                "weights": [float(value) for value in X[i]],  # Convert to float and store blendshape values
                "time": i / 30 + offset,  # Assuming 60 FPS, adjust this if different FPS is used
                "rotation": []  # Rotation is left as an empty list
            }

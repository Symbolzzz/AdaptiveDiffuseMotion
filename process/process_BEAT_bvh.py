...
from process.tool import calculate_mfcc, calculate_spectrogram, extract_onsets, extract_prosodic_features
...
import joblib as jl
import textgrid
from process.utils.data_utils import SubtitleWrapper, normalize_string
import csv
import io
...
import pickle
from tqdm import tqdm
from process.anim import bvh, quat, txform
...
def process_bvh_bugfix(gesture_filename):  # TODO: 20230723 not working yet
    ...
    jl.dump(
        data_pipe,
        os.path.join("./resource", "data_pipe_30fps_speaker" + speaker + ".sav"),
    )
...
# TODO：合成 json 文件
def expre2json(save_path, filename_prefix, expression):
    writer = JsonWriter()

    out_json_path = os.path.join(save_path, filename_prefix + "_generated.json")
    writer.write(X=expression, ofile=out_json_path)


def process_bvh(gesture_filename):
    all_poses, parents, dt, order, njoints = preprocess_animation(
        gesture_filename, fps=30
    )
    return all_poses


def interpolate_15fps_to_30fps(gesture_15fps):
    len_15fps, dim = gesture_15fps.shape  # 获取原始的长度和维度

    # 创建新的数组，长度翻倍
    len_30fps = 2 * len_15fps  # 新的长度是原来的两倍
    gesture_30fps = np.zeros((len_30fps, dim))

    # 将原始帧放到新数组中的偶数位置
    gesture_30fps[::2, :] = gesture_15fps  # 位置 0, 2, 4, ..., 2*len 都是原来的帧

    # 线性插值，填充中间帧
    for i in range(1, len_30fps, 2):
        if i // 2 + 1 < len_15fps:
            # 插值两帧之间的均值
            gesture_30fps[i, :] = (
                gesture_15fps[i // 2, :] + gesture_15fps[i // 2 + 1, :]
            ) / 2
        else:
            # 如果是最后一帧，直接复制最后一帧数据
            gesture_30fps[i, :] = gesture_15fps[i // 2, :]

    return gesture_30fps


def pose2bvh_bugfix(
    save_path, filename_prefix, poses, pipeline="./resource/data_pipe_30fps.sav"
):
    writer = BVHWriter()
    ...
    out_euler = np.zeros((out_poses.shape[0], out_poses.shape[1] * 3))  # (1922, 228)
...
def euler2bvh(
    ...
    bvh_data = pipeline.inverse_transform([poses])
...
def euler2pose(euler):
    # euler -> rotation matrix
    euler = euler.reshape((euler.shape[0], -1, 3))  # reshape to [frames, num_joints, 3]
    out_matrix = np.zeros(
        (euler.shape[0], euler.shape[1], 9)
    )  # [frames, num_joints, 9]

    for i in range(euler.shape[0]):  # frames
        r = R.from_euler(
            order, euler[i], degrees=True
        )  # Convert euler angles to rotation matrix
        out_matrix[i] = r.as_matrix().reshape(
            euler.shape[1], 9
        )  # Reshape 3x3 matrix to a flat vector

    out_matrix = out_matrix.reshape(
        (euler.shape[0], -1)
    )  # Reshape to [frames, num_joints * 9]

    return out_matrix


def pose2euler(poses):
    # smoothing
    n_poses = poses.shape[0]
    out_poses = np.zeros((n_poses, poses.shape[1]))
...
def pose2euler2(poses):
    out_eulers = []
    total = len(poses)  # 获取总数

    # 使用 tqdm 显示进度条，定制格式以显示当前处理的个数和剩余个数
    with tqdm(
        total=total, desc="Converting poses to euler angles", unit="pose"
    ) as pbar:
        for pose in poses:
            out_euler = pose2euler(pose)
            out_eulers.append(out_euler)
            pbar.update(1)  # 更新进度条，增加一个处理的个数
...
def wavlm_init(wavlm_model_path, device=torch.device("cuda:0")):
    ...
    from process.WavLM.WavLM import WavLM, WavLMConfig

    checkpoint = torch.load(
        wavlm_model_path, map_location=torch.device("cpu")
...
def wav2wavlm(model, wav_input_16khz, cfg, device=torch.device("cuda:0")):
    with torch.no_grad():
        ...
        wav_input_16khz = torch.nn.functional.pad(
            wav_input_16khz, (0, chunk_len * num_chunks - wav_len)
        )
...
def make_gesture_dataset(
    ...
    textraw_save_path = os.path.join(save_path, "textraw_BEAT")
    sem_save_path = os.path.join(save_path, "sem_BEAT")
    wav_save_path = os.path.join(save_path, "wav_BEAT")
    facial_save_path = os.path.join(save_path, "facial_BEAT")
    emo_save_path = os.path.join(save_path, "emo_BEAT")
    ...
    if not os.path.exists(textraw_save_path):
        os.makedirs(textraw_save_path)
    if not os.path.exists(facial_save_path):
        os.makedirs(facial_save_path)
    if not os.path.exists(sem_save_path):
        os.makedirs(sem_save_path)
    if not os.path.exists(emo_save_path):
        os.makedirs(emo_save_path)
    ...
    if not preload:
        for speaker in os.listdir(base_path):
            ...
            for v_i, bvh_file in enumerate(bvh_files):
                ...
                # save audio
                if os.path.exists(os.path.join(wav_save_path, name + ".npy")):
                    print(f"wav {name} exist")
                else:
                    wavpath = bvh_file[:-4] + ".wav"
                    a, sr = librosa.load(wavpath, sr=16000)
                    np.save(os.path.join(wav_save_path, name + ".npy"), a)
                ...
                if os.path.exists(os.path.join(text_save_path, name + ".npy")):
                    ...
                    Grid2tsv(tsvpath)
                    tsv = load_tsv(
                        tsvpath.replace(".TextGrid", ".tsv"), word2vector, clip_len
                    )
                ...
                # process raw text, this is used for CaMN model
                # word_each_file = []
                # if os.path.exists(os.path.join(textraw_save_path, name + ".npy")):
                #     print(f"text raw file {name} exist")
                # else:
                #     text_file = bvh_file[:-4] + ".TextGrid"
                #     try:
                #         tgrid = tg.TextGrid.fromFile(text_file)
                #     except:
                #         print(
                #             f"# ---- file not found for Word {text_file}, skip all files with the same id ---- #"
                #         )
                #         continue
                #     # the length of text file are reduce to the length of motion file, for x_x_a or x_x_b
                #     for i in range(poses.shape[0]):
                #         found_flag = False
                #         current_time = i / pose_fps
                #         for word in tgrid[0]:
                #             word_n, word_s, word_e = (
                #                 word.mark,
                #                 word.minTime,
                #                 word.maxTime,
                #             )
                ...
                #                 else:
                #                     word_each_file.append(
                #                         lang_model.get_word_index(word_n)
                #                     )
                #                 found_flag = True
                #                 break
                #             else:
                ...
                #     np.save(
                #         os.path.join(textraw_save_path, name + ".npy"), word_each_file
                #     )

                # process emotion
                emo_each_file = []
                if os.path.exists(os.path.join(emo_save_path, name + ".npy")):
                    print(f"emo {name} exist")

                else:
                    emo_file = bvh_file[:-4] + ".csv"
                    try:
                        emo_all = pd.read_csv(
                            emo_file,
                            sep=",",
                        ...
                        print(
                            f"# ---- file not found for Emo {name}, skip all files with the same id ---- #"
                        )
                        continue
                    for i in range(poses.shape[0]):
                        found_flag = False
                        for j, (start, end, score) in enumerate(
                            ...
                            if start <= current_time and current_time <= end:
                                ...
                                continue
                        if not found_flag:
                            emo_each_file.append(0)
                    emo_each_file = np.array(emo_each_file)
                    np.save(os.path.join(emo_save_path, name + ".npy"), emo_each_file)

                # process sem
                sem_each_file = []
                if os.path.exists(os.path.join(sem_save_path, name + ".npy")):
                    print(f"sem {name} exist")

                else:
                    sem_file = bvh_file[:-4] + ".txt"
                    try:
                        sem_all = pd.read_csv(
                            sem_file,
                            sep="\t",
                        ...
                        print(
                            f"# ---- file not found for Sem {name}, skip all files with the same id ---- #"
                        )
                        continue
                    ...
                    np.save(os.path.join(sem_save_path, name + ".npy"), sem_each_file)

    if preload:
        with h5py.File(f"BEAT_" + version + "_" + load_type + ".h5", "w") as h5:
            total_index = 0
            for speaker in os.listdir(base_path):
                # if speaker not in ['2', '10']:
                #     continue
                ...
                for v_i, bvh_file in enumerate(bvh_files):
                    name = os.path.split(bvh_file)[1][:-4]
                    if name == "2_scott_0_1_1":
                        continue  # pick a small files to test
                    print(f"Processing {v_i + 1}/{len(bvh_files)}: {name}")
                    g_data = h5.create_group(str(total_index))

                    poses = np.load(os.path.join(motion_save_path, name + ".npy"))
                    audio = np.load(os.path.join(audio_save_path, name + ".npy"))
                    tsv = np.load(os.path.join(text_save_path, name + ".npy"))
                    textraw = np.load(os.path.join(textraw_save_path, name + ".npy"))

                    expressions = np.load(os.path.join(facial_save_path, name + ".npy"))
                    wav = np.load(os.path.join(wav_save_path, name + ".npy"))
                    emo = np.load(os.path.join(emo_save_path, name + ".npy"))
                    # TODO：增加面部表情读取
                    clip_len = min(
                        poses.shape[0],
                        int(wav.shape[0] / 16000 * 30),
                        tsv.shape[0],
                        expressions.shape[0],
                        audio.shape[0],
                        textraw.shape[0],
                    ...
                    wav = wav[: int(clip_len / 30 * 16000)]
                    tsv = tsv[:clip_len]
                    audio = audio[:clip_len]
                    emo = emo[:clip_len]
                    textraw = textraw[:clip_len]
                    ...
                    g_data.create_dataset("audio", data=audio, dtype=np.float32)
                    g_data.create_dataset("text", data=tsv, dtype=np.float32)
                    # TODO：增加面部表情数据集构建
                    g_data.create_dataset(
                        "expression", data=expression, dtype=np.float32
                    )
                    g_data.create_dataset("emotion", data=emo, dtype=np.float32)
                    g_data.create_dataset("wav", data=wav, dtype=np.float32)
                    g_data.create_dataset("textraw", data=textraw, dtype=np.float32)
...
def main():
    ...
    parser.add_argument("--load_type", type=str, default="t")

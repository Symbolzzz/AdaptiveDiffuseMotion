...
class AdaptiveDiffuseMotion(nn.Module):
    def __init__(self, modeltype, njoints, nfeats, nexpressions,
                 latent_dim=256, ff_size=1024, num_layers=8, num_heads=4, dropout=0.1,
                 ablation=None, activation="gelu", legacy=False, data_rep='rot6d', dataset='amass', clip_dim=512,
                 arch='trans_enc', emb_trans_dec=False, n_seed=1, device='cpu', 
        ...
        self.embed_text1 = nn.Linear(self.njoints, self.audio_feat_dim)
        self.embed_text2 = nn.Linear(self.nexpressions, self.audio_feat_dim)

        # 这个的作用是将隐藏维度映射到 njoints
        self.output_process = OutputProcess(self.data_rep, self.input_feats, self.latent_dim, self.njoints,
                                            self.nfeats)
        self.output_process2 = OutputProcess(self.data_rep, self.nexpressions, self.latent_dim, self.nexpressions,
                                            self.nfeats)
        ...
        self.input_process1 = InputProcess(self.data_rep, self.nexpressions, self.latent_dim)
    ...
    def generate_output(self, x, embed_text, emb_t, embed_style, enc_text, y, input_process, output_process):
        ...
        # self-attention
        x_ = input_process(x)  # [2, 135, 1, 240] -> [240, 2, 256]
        ...
        output = output_process(output)  # [bs, njoints, nfeats, nframes]
        return output
    ...
    def forward(self, x, timesteps, y=None):
        ...
        """
        # x 是一个加了噪声的真值
        bs, njoints_add_nexpressions, nfeats, nframes = x.shape      # 64, 251, 1, 196
        # 这里是共享的参数，speaker ID, audio, text, timestep
        ...
        embed_text1 = self.embed_text1(y['seed gesture'].squeeze(2).permute(0, 2, 1)).permute(1, 0, 2)  # (30, bs, 256-64)
        embed_text2 = self.embed_text2(y['seed expression'].squeeze(2).permute(0, 2, 1)).permute(1, 0, 2)  # (30, bs, 256-64)
        # noisy gesture / expression
        x_1 = x[:, :2052]
        x_2 = x[:, 2052:]
        
        output1 = self.generate_output(x_1, embed_text1, emb_t, embed_style, enc_text, y, self.input_process, self.output_process) # [350, 2052, 1, 150]
        output2 = self.generate_output(x_2, embed_text2, emb_t, embed_style, enc_text, y, self.input_process1, self.output_process2)

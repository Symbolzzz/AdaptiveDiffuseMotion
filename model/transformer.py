    ...
    def __init__(self,
        ...
        if opt.model_base == "transformer_encoder":
            ...
            if opt.expCondition_gesture_only != None:
                pre_proj_dim = latent_dim + aud_latent_dim + opt.facial_dim
            elif self.opt.gesCondition_expression_only:
                pre_proj_dim = latent_dim + aud_latent_dim + opt.motion_dim
            else:
...
class LinearTemporalDiffusionTransformerLayer(nn.Module):
    def __init__(self,
        ...
        if opt.expCondition_gesture_only != None:
            pre_proj_dim = pre_proj_dim + aud_latent_dim + opt.facial_dim
        elif self.opt.gesCondition_expression_only:
            pre_proj_dim = pre_proj_dim + aud_latent_dim + opt.motion_dim
        else:
...
class MotionTransformer(nn.Module):
    def __init__(self,
        ...
        if opt.expCondition_gesture_only != None:
            self.pre_proj_dim = self.pre_proj_dim + aud_latent_dim + opt.facial_dim
        elif self.opt.gesCondition_expression_only:
            self.pre_proj_dim = self.pre_proj_dim + aud_latent_dim + opt.motion_dim
        else:
    ...
    def forward(self, x, timesteps, audio_emb, length, person_id, add_cond={}, pe_type="learnable", y=None, block=None):
        ...
        if self.opt.expCondition_gesture_only != None:
            audio_emb, exp_cond = torch.split(audio_emb, [self.audio_dim, self.opt.facial_dim], dim=-1)
        elif self.opt.gesCondition_expression_only:
            audio_emb, exp_cond = torch.split(audio_emb, [self.audio_dim, self.opt.motion_dim], dim=-1)

...
class GaussianDiffusion:
    ...
    def masked_l2_2(self, a, b, mask):
        # Assuming a.shape == b.shape == [bs, seqlen, dim]
        # Assuming mask.shape == [bs, seqlen, 1]

        # Compute the smooth L1 loss element-wise between a and b
        loss = self.smooth_l1_loss(a, b)  # [bs, seqlen, dim]

        # Apply mask and sum over the unmasked elements
        masked_loss = loss * mask.float()  # Element-wise multiplication with mask
        total_loss = sum_flat(masked_loss)  # Sum over all elements

        # Calculate the number of unmasked elements
        n_entries = a.shape[2]  # Feature dimension
        non_zero_elements = sum_flat(mask) * n_entries  # Sum of non-zero elements in mask

        # Calculate mean squared error over unmasked elements
        mse_loss_val = total_loss / non_zero_elements
        return mse_loss_val
    ...
    def p_mean_variance(
        ...
        # gesture, expression = th.split(x, 2052, dim=-1)
        # sqrt_recip_alphas_cumprod_t = _extract_into_tensor(self.sqrt_recip_alphas_cumprod, t, expression.shape)
        # sqrt_recipm1_alphas_cumprod_t = _extract_into_tensor(self.sqrt_recipm1_alphas_cumprod, t, expression.shape)
        # sqrt_alphas = [sqrt_recip_alphas_cumprod_t, sqrt_recipm1_alphas_cumprod_t]
        # model_kwargs["sqrt_alphas"] = sqrt_alphas
        # model_kwargs["sqrt_alphas"] = None
        
        model_output = model(x, self._scale_timesteps(t), **model_kwargs)
        if isinstance(model_output, tuple):
            model_output = torch.cat((model_output[0], model_output[1]), dim=1)
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:     # LossType.MSE
            ...
            model_output1, model_output2 = model(x_t, self._scale_timesteps(t), **model_kwargs)  # [bs, njoints, nfeats, nframes]
            ...
            target_gesture, target_expression = torch.split(target, 2052, dim=1)
            
            # assert model_output1.shape == target_gesture.shape  # [bs, njoints, nfeats, nframes]
            # assert model_output2.shape == target_expression.shape
            ...
            terms["blend_mse"] = self.masked_l2(target_expression, model_output2, mask)
            terms["rot_mse"] = self.masked_l2(target_gesture, model_output1, mask)
            ...
            terms["loss1"] = terms["rot_mse"] + terms.get('vb', 0.) +\
                            (self.lambda_vel * terms.get('vel_mse', 0.)) +\
                            (self.lambda_rcxyz * terms.get('rcxyz_mse', 0.)) + \
                            (self.lambda_fc * terms.get('fc', 0.))
                            
            terms["loss2"] = terms["blend_mse"]
    ...
    def training_losses_2(self, model, x_start, t, model_kwargs=None, noise=None):
        """
        Compute training losses for a single timestep.
        ...
        if noise is None:
            noise = th.randn_like(x_start)
        x_t = self.q_sample(x_start, t, noise=noise) # x_t and x_start have the same size
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            # NOTE 这部分原函数没有
            gesture, expression = th.split(x_start, 2052, dim=-1)
            sqrt_recip_alphas_cumprod_t = _extract_into_tensor(self.sqrt_recip_alphas_cumprod, t, expression.shape)
            sqrt_recipm1_alphas_cumprod_t = _extract_into_tensor(self.sqrt_recipm1_alphas_cumprod, t, expression.shape)
            sqrt_alphas = [sqrt_recip_alphas_cumprod_t, sqrt_recipm1_alphas_cumprod_t]
            model_kwargs["sqrt_alphas"] = sqrt_alphas
            #     model_output = model(x_t, self._scale_timesteps(t), sqrt_alphas, **model_kwargs) # model_ouput and x_t have the same size
            # model_kwargs["sqrt_alphas"] = None
            # else:
            model_output = model(x_t, self._scale_timesteps(t), **model_kwargs) # model_ouput and x_t have the same size
            model_output1, model_output2 = th.split(model_output, 2052, dim=-1)
            ...
            # if self.model_mean_type == ModelMeanType.START_X and 'outpainting_mask' in model_kwargs['y'].keys() and 'gt' in model_kwargs['y'].keys():
            #     outpainting_mask, inpainted_motion = model_kwargs['y']['outpainting_mask'], model_kwargs['y']['gt']
            #     
            #     outpainting_mask, inpainted_motion = outpainting_mask.to(model_output.device), inpainted_motion.to(model_output.device).float()
            ...
            target_gesture, target_expression = th.split(target, 2052, dim=-1)
            # terms["loss"] = mean_flat((target - model_output) ** 2).view(-1, 1).mean(-1) # [200, 150, 2205]
            terms["loss1"] = self.masked_l2_2(target_gesture, model_output1, mask)
            terms["loss2"] = self.masked_l2_2(target_expression, model_output2, mask)
            # if "vb" in terms:
        ...
        #     if self.model_mean_type == ModelMeanType.START_X:
        #         terms["target_vel"] = target[:, :-1, :] - target[:, 1:, :]
        #         terms["pred_vel"] = model_output[:, :-1, :] - model_output[:, 1:, :]
        #     elif self.model_mean_type == ModelMeanType.EPSILON:
        #         terms['target_x0'] = x_start
        #         terms['pred_x0'] = self._predict_xstart_from_eps(x_t=x_t, t=t, eps=model_output)
        #         terms['target_vel'] = x_start[:, :-1, :] - x_start[:, 1:, :]
        #         terms["pred_vel"] = terms['pred_x0'][:, :-1, :] - terms['pred_x0'][:, 1:, :]
